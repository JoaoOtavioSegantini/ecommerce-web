...
const Search: React.FC = () => {
  ...
  const { data: categoriesData, error: categoriesError } = useSwr(
    "/storefront/v1/categories?length=999",
    ...
    <MainComponent>
      <div>
        <div className="text-center mt-4">
          <h3 className={styles.title}>Resultados da Pesquisa</h3>
        </div>
        ...
        <Row className="text-center col-md-6 offset-md-3">
          ...
          <Col xs={2}>
            <i
              ...
              className={styles.search_icon + " fa fa-search"}
        ...
        <Row>
          <Col sm={6} xs={12} className={styles.results}>
            <span>{data?.meta?.total} resultado(s)</span>
          </Col>
        ...
        <div className="mt-4">
          {router?.query?.search && (
            <h5>Resultados para {router?.query?.search}</h5>
          )}
        </div>
      </div>
      ...
      {data?.meta?.total! > 0 && (
        <Pagination className={styles.pagination} {...data?.meta!} />
      )}
    </MainComponent>
  );
};
